          ...
          <ul class="navbar-nav">
            ...
            <template v-for="(item, index) of menuItems" :key="index">
              <li class="nav-item dropdown">
                ...
                <BButton v-if="item.subItems" class="nav-link dropdown-toggle arrow-none" id="topnav-components"
                  role="button">
                ...
                <div class="dropdown-menu" aria-labelledby="topnav-dashboard" v-if="hasItems(item)"
                  ...
                  <template v-for="(subitem, index) of item.subItems">
                    <router-link class="col dropdown-item side-nav-link-ref" :key="index"
                      v-if="item.subItems.length < 12 && !hasItems(subitem)" :to="subitem.link">{{ subitem.label
                      }}</router-link>
                    <div v-if="item.subItems.length > 11" :key="index">
                      <BRow v-if="index % 3 == 0">
                        <BCol lg="4"><router-link class="dropdown-item side-nav-link-ref" :to="subitem.link">{{
                          item.subItems[index].label }}</router-link></BCol>
                        <BCol lg="4" v-if="item.subItems[index + 1].link"><router-link
                            class="dropdown-item side-nav-link-ref" :to="item.subItems[index + 1].link">{{
                              tem.subItems[index + 1].label }}</router-link></BCol>
                        <BCol lg="4" v-if="item.subItems[index + 2]"><router-link
                            class="dropdown-item side-nav-link-ref" :to="item.subItems[index + 2].link">{{
                              item.subItems[index + 2].label }}</router-link></BCol>
                      </BRow>
                    </div>
                      ...
                      <div class="dropdown-menu">
                        <template v-for="(subSubitem, index) of subitem.subItems">
                          <router-link class="dropdown-item side-nav-link-ref" :key="index" v-if="!hasItems(subSubitem)"
                            :to="subSubitem.link">{{ subSubitem.labelledby }}</router-link>
                            ...
                            <div class="dropdown-menu">
                              <template v-for="(
                                ...
                                <router-link class="dropdown-item side-nav-link-ref" :to="subSubSubitem.link"
                                  routerLinkActive="active">{{ subSubSubitem.label }}</router-link>
                              </template>
                            </div>
                          </div>
                        </template>
